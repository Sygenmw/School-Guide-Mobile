...
class MultiTrackTween extends Animatable<Map<String, dynamic>> {
  ...
  MultiTrackTween(List<Track> tracks)
      : assert(tracks.isNotEmpty, "Add a List<Track> to configure the tween."),
        assert(tracks.where((track) => track.items.isEmpty).isEmpty, "Each Track needs at least one added Tween by using the add()-method.") {
  ...
  void _computeTrackTweens(List<Track> tracks) {
    tracks.forEach((track) {
      ...
      final sequenceItems = track.items.map((item) => TweenSequenceItem(tween: item.tween!, weight: item.duration.inMilliseconds / _maxDuration)).toList();
